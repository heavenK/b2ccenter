...
function loadgroup(lineid) {
	url = 'http://www.myerpcenter.com/index.php?s=/Server/getxianlubyID/erpxianluID/' + _lineid;
...
function computeprice(ctrl, type) {
    ...
    clickurl = "http://www.myerpcenter.com/index.php?s=/Yuding/adddingdan/chanpinID/"+group.groupid+"/chengrenshu/"+pernum+"/ertongshu/"+childnum;
    if (totalnum > group.renshu || group.renshu == 0 || pernum == 0) {
        $("#Popupbox .nextstep input").attr("src", "./testpage001/xiayibu_gray.gif");
        $("#Popupbox .nextstep input").unbind().bind("click", function() { alert("请检查预订人数，不可超过该团的剩余名额,并且成人数不能为0！"); });
    }
    else {
        $("#Popupbox .nextstep input").attr("src", "./testpage001/nextstep.gif");
